...
class OperationTest extends AnyFlatSpec with Matchers {
  ...
  it should "cut negative value" in {
    ...
    Cut(-2).performOp(deck) shouldBe List(5, 6, 1, 2, 3, 4)
    Cut(-4).performOp(deck) shouldBe List(3, 4, 5, 6, 1, 2)
  }

  it should "deal into new stack" in {
    val deck = List(1, 2, 3, 4, 5, 6)

    DealIntoNewStack.performOp(deck) shouldBe List(6, 5, 4, 3, 2, 1)
  }

  it should "deal with increment" in {
    val deck = SlamShuffle.generateFactoryOrderDeck(10)

    DealWithIncrement(3).performOp(deck) shouldBe List(0, 7, 4, 1, 8, 5, 2, 9, 6, 3)
  }
