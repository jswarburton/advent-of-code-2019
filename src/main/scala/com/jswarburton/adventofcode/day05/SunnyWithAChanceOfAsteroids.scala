  ...
  def run(data: List[Int], inputInstruction: Int): List[Int] = {
    ...
    def helper(latest: IndexedSeq[Int],
      ...
      opCode match {
        // Add
        case 1 =>
          val newValue = read(mode1, 0) + read(mode2, 1)
          val newMemory = write(2, newValue)
        ...
        case 5 =>
          if (read(mode1, 0) != 0) helper(pointer = read(mode2, 1), outputs = outputs, inputInstructions = inputInstructions, latest = latest)
        ...
        case 7 =>
          val newValue = if (read(mode1, 0) < read(mode2, 1)) 1 else 0
          val newMemory = write(2, newValue)
